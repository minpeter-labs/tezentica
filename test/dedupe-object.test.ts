...
describe("claimSlackMessageOnce", () => {
  it("allows only the first delivery for a Slack message key", async () => {
    ...
    const first = await claimSlackMessageOnce(
      storage,
      "T123:C123:1710000000.000100"
    );
    const second = await claimSlackMessageOnce(
      storage,
      "T123:C123:1710000000.000100"
    );
...
describe("MessageDedupeObject", () => {
  it("accepts a claim request and rejects duplicate claims", async () => {
    ...
    const first = await object.fetch(
      createClaimRequest("T123:C123:1710000000.000100")
    );
...
class InMemoryDedupeStorage {
  ...
  transaction<T>(
    closure: (txn: InMemoryDedupeTransaction) => Promise<T>
  ): Promise<T> {
